...
public class TestMixingQuestionServiceImpl extends BaseServiceImpl<TestMixingQuestion, TestMixingQuestionDTO, TestMixingQuestionRepository> implements TestMixingQuestionService {
    ...
    public List<TestMixingQuestionDTO> findAllTestMixingQuestionsByTestIdAndStatus(String testId, StatusEnum status) {
        if (status == null) {
        ...
        List<TestMixingQuestion> list = repository.findAllByTest_IdAndStatus(testId, status);

        return list.stream()
                .map(entity -> {
                    TestMixingQuestionDTO dto = convertToDTO(entity);
                    dto.setAnswers(testMixingAnswerService.findAllByQuestionIdAndStatus(entity.getId(), status));
                    return dto;
                })
